...
# ============================================================================
# NEXUS v5.0 ELITE CYBER-INDUSTRIAL OLED THEME
# ============================================================================
st.markdown("""
<style>
...
:root {
    ...
    --glow-amber: 0 0 20px rgba(255, 159, 0, 0.3);
}
...
@keyframes pulse-critical {
    ...
    50% { opacity: 0.7; box-shadow: 0 0 24px rgba(255, 51, 102, 0.9); }
}

@keyframes pulse-warning {
    0%, 100% { opacity: 1; box-shadow: 0 0 8px rgba(255, 159, 0, 0.6); }
    50% { opacity: 0.7; box-shadow: 0 0 24px rgba(255, 159, 0, 0.9); }
}

@keyframes pulse-optimal {
    0%, 100% { opacity: 1; box-shadow: 0 0 8px rgba(0, 255, 136, 0.6); }
    50% { opacity: 0.7; box-shadow: 0 0 24px rgba(0, 255, 136, 0.9); }
}
...
.status-pulse.online { background: var(--accent-green); box-shadow: var(--glow-green); }
.status-pulse.warning { background: var(--accent-amber); box-shadow: var(--glow-amber); }
...
.alarm-badge.warning { background: rgba(255, 159, 0, 0.25); color: #FF9F00 !important; }
.alarm-badge.optimal { background: rgba(0, 255, 136, 0.25); color: #00FF88 !important; }

.header-bar {
    background: linear-gradient(90deg, rgba(18, 18, 18, 0.95), rgba(10, 10, 10, 0.98));
    backdrop-filter: var(--glass-blur);
...
.stButton > button:hover {
    background: linear-gradient(135deg, rgba(0, 229, 255, 0.25), rgba(0, 255, 136, 0.15)) !important;
    box-shadow: var(--glow-cyan) !important;
...
.auto-tune-card {
    background: linear-gradient(145deg, rgba(0, 229, 255, 0.06), rgba(0, 255, 136, 0.04));
...
.hw-status-dot.connected { background: var(--accent-green); box-shadow: var(--glow-green); }
.hw-status-dot.disconnected { background: var(--accent-crimson); box-shadow: 0 0 8px var(--accent-crimson); }
.hw-status-dot.reconnecting { background: var(--accent-amber); box-shadow: var(--glow-amber); animation: pulse 1s infinite; }
...
.role-badge.operator { background: rgba(0, 229, 255, 0.12); color: #00E5FF !important; border: 1px solid rgba(0, 229, 255, 0.3); }
.role-badge.engineer { background: rgba(0, 255, 136, 0.12); color: #00FF88 !important; border: 1px solid rgba(0, 255, 136, 0.3); }
.role-badge.admin { background: rgba(168, 85, 247, 0.12); color: #A855F7 !important; border: 1px solid rgba(168, 85, 247, 0.3); }
...
.arch-doc-card p, .arch-doc-card li {
    font-size: 12px;
    color: var(--text-secondary) !important;
    line-height: 1.6;
}
...
# ============================================================================
# PLOTLY CHART CONFIGURATION - ELITE DARK MODE
# ============================================================================
PLOTLY_TEMPLATE = {
    "layout": {
        "paper_bgcolor": "rgba(0,0,0,0)",
        "plot_bgcolor": "rgba(10,10,10,0.6)",
        "font": {"family": "Inter, sans-serif", "color": "#7A7A7A", "size": 11},
        "xaxis": {
        ...
        "hoverlabel": {
            "bgcolor": "#111111",
            "bordercolor": "#00E5FF",
            "font": {"family": "JetBrains Mono", "size": 12, "color": "#F0F0F0"}
        }
    }
}
...
with st.sidebar:
    ...
    with st.expander("System Architecture & Data Flow", expanded=False):
        st.markdown("""
        <div class="arch-doc-card">
            <h4>DATA SOURCES</h4>
            <ul style="padding-left:16px;margin:0;">
                <li><strong style="color:#00E5FF;">Digital Twin Engine</strong> - Python PID simulation with configurable process models (Thermal, Hydraulic, Motor, Pressure)</li>
                <li><strong style="color:#00E5FF;">Serial/USB Gateway</strong> - Direct hardware I/O via pyserial, reads JSON telemetry frames from embedded controllers</li>
                <li><strong style="color:#00E5FF;">MQTT IoT Gateway</strong> - Publish/subscribe telemetry via Mosquitto/HiveMQ brokers, supports QoS levels</li>
                <li><strong style="color:#00E5FF;">Modbus TCP Gateway</strong> - Industrial PLC communication via holding registers, scale factor normalization</li>
            </ul>
        </div>
        <div class="arch-doc-card">
            <h4>PID CONTROL ENGINE</h4>
            <ul style="padding-left:16px;margin:0;">
                <li><strong style="color:#00FF88;">Ziegler-Nichols Auto-Tune</strong> - Classic, PI-Optimized, and Modern PID tuning presets</li>
                <li><strong style="color:#00FF88;">Real-Time KPIs</strong> - IAE, ISE, Overshoot, Settling Time, Rise Time, Steady-State Error</li>
                <li><strong style="color:#00FF88;">Alarm Engine</strong> - Critical/Warning/Optimal thresholds with live status banners</li>
            </ul>
        </div>
        <div class="arch-doc-card">
            <h4>RBAC SECURITY</h4>
            <ul style="padding-left:16px;margin:0;">
                <li><strong style="color:#FF9F00;">Operator</strong> - Read-only dashboard, no tuning or override access</li>
                <li><strong style="color:#FF9F00;">Control Engineer</strong> - Full PID tuning, manual override, report export</li>
                <li><strong style="color:#FF9F00;">Plant Admin</strong> - Full system access including data source configuration</li>
            </ul>
        </div>
        """, unsafe_allow_html=True)
...
fig.add_trace(go.Scatter(x=time_data, y=pv_data, name="Process Variable", line=dict(color="#00E5FF", width=2.5), fill="tozeroy", fillcolor="rgba(0, 229, 255, 0.06)", hovertemplate="t=%{x}s<br>PV=%{y:.3f}<extra></extra>"), row=1, col=1)
fig.add_trace(go.Scatter(x=time_data, y=output_data, name="Control Output", line=dict(color="#00FF88", width=2), fill="tozeroy", fillcolor="rgba(0, 255, 136, 0.06)", hovertemplate="t=%{x}s<br>u=%{y:.1f}%<extra></extra>"), row=2, col=1)

fig.update_layout(**PLOTLY_TEMPLATE["layout"], height=520, showlegend=True, legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1, font=dict(size=11, color="#7A7A7A")), margin=dict(l=50, r=30, t=40, b=30))
...
st.markdown('<div class="tactical-divider"></div>', unsafe_allow_html=True)
st.markdown("""
<div style="text-align:center;padding:20px 0;font-family:'JetBrains Mono',monospace;font-size:11px;color:#7A7A7A;">
    NEXUS v5.0 Elite | Industrial SCADA & Digital Twin Platform | Developed by <span style="color:#00E5FF;">Ali Nasreddine Benseffa</span> | 2026
</div>
""", unsafe_allow_html=True)
